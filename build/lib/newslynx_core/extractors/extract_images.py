...
def square_img(img):
  ...
  while y > x:
    ...
    if img_entropy(bottom) < img_entropy(top):
...
def img_from_url(
  ...
  while True:
    try:
      ...
      cur_try += 1
      if cur_try >= retries:
        print('error while fetching: %s refer: %s' % (img_url, referer))
        return nothing
  ...
  def valid_img(img_url, dim):
    ...
    area = calc_img_area(img_url, dim)
    return area > IMG_MIN_AREA

class ImageExtractor:
  """
  Class for Extracting Top Images 
  Taken From Reddit Source Code
  """
  def __init__(self, **kwargs):
    self.referer = kwargs.get('referer', None)
    self.top_img = u'' # if not top_img else top_img
  ...
  def fetch_img_dim(self, img_url):
    ...
    of an img
    """
    return img_from_url(img_url, self.referer, dim=True)

  def fetch_img(self, img_url):
    """
    Fetch an img 
    """
    return img_from_url(img_url, self.referer) 
  ...
  def get_top_img(self, img_urls):
    ...
    # get largest img
    img_url = self.largest_image_url(img_urls)
    
    # if there's a valid img, get it.
    if img_url:
      content_type, img_str = fetch_img_url(img_url)

      # if we got an img parse it.
      if img_str:
        img = str_to_img(img_str)
        try:
          thumb = img_to_thumbnail(img)
        except IOError, e:
      ...
      return img_url, img_to_b64(img), img_to_b64(thumb)
